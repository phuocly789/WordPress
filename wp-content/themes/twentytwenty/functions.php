...
function get_estimated_reading_time($content = '', $wpm = 200)
{
	// Nếu không có content thì lấy content của post hiện tại
	if (empty($content)) {
	...
	$reading_time_text = sprintf(
		_n('%s minute read', '%s minutes read', $time, 'twentytwenty'),
		$time
	);
	// Hoặc đơn giản hơn nếu website chỉ có tiếng Việt:
	// $reading_time_text = $time . ' phút đọc';
